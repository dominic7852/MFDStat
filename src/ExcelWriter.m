classdef ExcelWriter < handle
    %EXCELWRITER Summary of this class goes here
    ...
    methods
        ...
        function delete(obj)
            invoke(obj.Excel.ActiveWorkbook,'Save');
            obj.Excel.Quit
            obj.Excel.delete
            clear Excel
        end
        
        function xlswrite(obj,file,data,sheet,range)
            % Set default values.
            Sheet1 = 1;
            ...
            try
                ...
                if ~isempty(file)
                    ...
                    file = obj.abspath(fullfile(Directory,[file ext]));
            ...
            try
                if ~exist(file,'file')
                    ...
                    ExcelWorkbook = obj.Excel.workbooks.Add;
                    ExcelWorkbook.SaveAs(file);
                    ExcelWorkbook.Close(false);
                end
                ...
                try % select region.
                    ...
                    message = obj.activate_sheet(sheet);
                    
                    % Select range in worksheet.
                    Select(Range(obj.Excel,sprintf('%s',range)));
                ...
                set(obj.Excel.selection,'Value',A);
                %ExcelWorkbook.Save
    ...
    methods (Access = private)
        function message = activate_sheet(obj, Sheet)
            % Activate specified worksheet in workbook.
            
            % Initialise worksheet object
            WorkSheets = obj.Excel.sheets;
            message = struct('message',{''},'identifier',{''});
        ...
        %------------------------------------------------------------------------------
        function newsheet = addsheet(obj, WorkSheets,Sheet)
        ...
        %------------------------------------------------------------------------------
        function [absolutepath]=abspath(obj, partialpath)
